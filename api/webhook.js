...
// IMPORTANT: Vercel's default body parser converts the incoming request
// into a JS object before this handler ever runs. Razorpay signs the
// EXACT raw bytes of the request body, so re-stringifying req.body
// with JSON.stringify() almost never matches byte-for-byte (key order,
// spacing, number formatting can all differ) — this silently breaks
// signature verification and causes every delivery to fail with 400,
// which is why Razorpay auto-disables the webhook after repeated
// failures. Disabling the body parser and reading the raw stream
// ourselves fixes this.
...
module.exports = async function handler(req, res) {
  ...
  try {
    // Read the raw, unparsed request body — this is what Razorpay
    // actually signed, so we verify against this, not a reconstructed object.
    ...
    // Only parse the JSON AFTER the signature check has passed.
